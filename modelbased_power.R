################################################################################
# example function to simulate data for model-based power (true beta file needed!!!)
...
modelbased_power <- function(beta, clstr_size, clstr,lambda, q, cmptrsk, dropout, tau_f, tau_c, m){
  
  truebeta<-read.csv(paste0('./true_beta/true_beta_hr',exp(beta),".csv"))
  truebeta<-truebeta[,-c(1,9)]
  dir.create("./modelbased_power")
    ...
    for (o2 in 1:p2){
      for (o1 in 1:p1){
        ...
        for (j in 1:m) {
          ...
          #Set up the true beta
          truebeta0<-truebeta[truebeta$tau1==tau_c0[o1]&truebeta$tau2==tau_f0[o2]&truebeta$lambda2==cmptrsk[o3],7]
          ...
          ms.coef<- data.frame(Coefficient = test.ms$coefficients[1],
                               ...
                               coverage = test.ms$coefficients[1]-q.95*sqrt(test.ms$var[1,1])<truebeta0[3]
                               & test.ms$coefficients[1]+q.95*sqrt(test.ms$var[1,1])>truebeta0[3], 
                               pvalue = summary(test.cox)$coefficient[5],
                               modelName = "ms")
          ms_cluster.coef <- data.frame(Coefficient = test.ms_cluster$coefficients[1],
                                        SE = sqrt(test.ms_cluster$var[1,1]), 
                                        coverage = test.ms_cluster$coefficients[1]-q.95*sqrt(test.ms_cluster$var[1,1])<truebeta0[3] & test.ms_cluster$coefficients[1]+q.95*sqrt(test.ms_cluster$var[1,1])>truebeta0[3],
                                        pvalue = summary(test.ms_cluster)$coefficient[1,6],
                                        modelName = "ms_cluster")
          cox.coef <- data.frame(Coefficient = summary(test.cox)$coefficients[1],
                                 SE = summary(test.cox)$coefficients[3], 
                                 coverage = confint(test.cox)[1]<truebeta0[1] & confint(test.cox)[2]>truebeta0[1], 
                                 pvalue = summary(test.cox)$coefficient[5],
                                 modelName = "Cox")
          cox_cluster.coef <- data.frame(Coefficient = summary(test.cox_cluster)$coefficients[1],
                                         ...
                                         coverage = confint(test.cox_cluster)[1]<truebeta0[1] & confint(test.cox_cluster)[2] >truebeta0[1], 
                                         pvalue = summary(test.cox_cluster)$coefficient[6],
                                         modelName = "Marginal Cox")
          crr.coef <- data.frame(Coefficient = test.crr$coef,
                                 SE = sqrt(test.crr$var),
                                 coverage = test.crr$coef -q.95*sqrt(test.crr$var)<truebeta0[2] & test.crr$coef +q.95*sqrt(test.crr$var) >truebeta0[2],
                                 pvalue = summary(test.crr)$coef[5],
                                 modelName = "Fine and Gray")
          zhou.coef <- data.frame(Coefficient = test.zhou$coef,
                                  SE = sqrt(test.zhou$var),
                                  coverage = test.zhou$coef -q.95*sqrt(test.zhou$var)<truebeta0[2] & test.zhou$coef +q.95*sqrt(test.zhou$var) >truebeta0[2],
                                  pvalue = summary(test.zhou)$coef[5],
                                  modelName = "Zhou")
        ...
        colnames(result)<-c("coefficient","SE","coverage","pvalue", "modelName",
                            "tau1","tau2","lambda1","lambda2","dropout","HR","mean.cluster.size","CV.cluster.size")
        close(pb1)
        write.csv(result,file = paste0('./modelbased_power/tau_f_',tau_f,"_tau_c_",tau_c,"_cmptrsk_",gamma,".csv"))
      }
    } 
  }
}
...
#################################################################################
#Call the function
modelbased_power(beta=beta, clstr_size=clstr_size, clstr=clstr,lambda =lambda, 
                 q = q, cmptrsk=cmptrsk, dropout=dropout, tau_f=tau_f, tau_c=tau_c, m=m)
#Combine the results into a single file
csv.list <- list.files(path="./modelbased_power/", pattern=".csv$", full.names=TRUE)
full<-NULL
for (i in 1:length(csv.list)) {
  update<-read.csv(csv.list[i])
  full<-rbind(full,update)
}
system("rm ./modelbased_power/*csv")
write.csv(full,file = paste0('./modelbased_power/[m]full_cluster',clstr,"_hr",exp(beta),".csv"))
